...
with open('flask/flask1/config.json','r', encoding='utf-8')  as c:
    paramiters=json.load(c)["paramiters"]

with open('flask/flask1/blogs.json','r', encoding='utf-8')  as s:
...
#     MAIL_PORT = '465',
#     MAIL_USE_SSL = True,
#     MAIL_USERNAME = paramiters['gmail_username'],
#     MAIL_PASSWORD = paramiters['gmail_password']
# )
# mail = Mail(panel)
...
def sign_in():
    if request.method == "POST":
        ...
        entry = Sign_in(first_name=fname, last_name=lname, phone_num=number, email=mail, birthday=birthday, age=age)
        db.session.add(entry)
        db.session.commit()
